...
function renderMap(detail) {
 ...
 <section class="panel map-command-panel">
   ...
   <div class="map-save-row"><p id="save-note" class="save-note">Salvo · Valores em R$ · Preço inicial → preço negociado → desconto automático.</p><span>${quoted}/${m.items.length} itens com cotação</span></div>
 ...
 <section class="panel quote-workbench">
   <div class="quote-workbench-head">
     <div><span class="section-kicker">ANÁLISE DE PREÇOS</span><h2>Mapa de cotação</h2><p class="muted">Compare cada fornecedor por item. Sem cotação, deixe o preço inicial vazio; sem negociação, deixe o preço negociado vazio. Frete não incluído.</p></div>
     <button id="request-quote" class="button primary" ${!m.suppliers.length||m.archived||detail.changes.length?'disabled':''}>Solicitar cotação por WhatsApp</button>
   </div>
   <div class="purchase-table map-quote-table"><table><thead><tr><th class="sticky-col origin-col">Hotel / SCI</th><th class="sticky-col item-col">Item / especificação</th><th class="sticky-col qty-col">Qtd.</th>${m.suppliers.map(s=>`<th class="supplier-heading"><span class="supplier-initial">${esc((s.name||'?').trim().charAt(0).toUpperCase()||'?')}</span><span>${esc(s.name)}</span></th>`).join('')}<th class="result-heading">Resultado</th></tr></thead><tbody>${m.items.map((i,idx)=>{const line=r.lines[idx],quotedCount=line.quotes.length;return `<tr class="quote-item-row"><td class="sticky-col origin-col origin-cell"><strong>${esc(i.company)}</strong><small>SCI ${esc(i.sci)}</small><small>${esc(i.buyer)}</small><span class="badge ${i.approval==='approved'?'approval-approved':'approval-waiting'}">${statusLabel[i.approval]}</span></td><td class="sticky-col item-col description"><strong>${esc(i.description)}</strong>${i.article?`<small>${esc(i.article)}</small>`:''}<span class="item-quote-progress ${quotedCount?'has-quotes':''}">${quotedCount}/${m.suppliers.length} fornecedores cotaram</span><details class="item-details"><summary>Observação / tipo de compra</summary><label>Tipo de compra<input data-type="${esc(i.id)}" value="${esc(i.purchase_type)}" placeholder="Opcional"></label><label>Observação<textarea data-note="${esc(i.id)}" rows="2" maxlength="2000" placeholder="Marca, especificação, entrega…">${esc(i.note)}</textarea></label></details></td><td class="sticky-col qty-col qty-cell"><strong>${qty(i.quantity)}</strong><small>${esc(i.unit)}</small></td>${m.suppliers.map(s=>{const q=m.quotes[i.id]?.[s.id]||{},calc=line.quotes.find(q=>q.supplier_id===s.id),isWinner=line.chosen?.supplier_id===s.id,isTie=line.state==='tie'&&line.winners.some(w=>w.supplier_id===s.id);const finalValue='negotiated' in q?(q.negotiated??''):(calc?.final_price??'');return `<td class="quote-cell ${isWinner?'is-winner':''} ${isTie?'is-tie':''}"><div class="quote-card">${isWinner?'<span class="best-offer-tag">Melhor oferta</span>':isTie?'<span class="tie-offer-tag">Empatada</span>':''}<div class="price-pair"><label>Preço inicial<input aria-label="Preço inicial ${esc(s.name)}" inputmode="decimal" data-price="${s.id}" data-item="${esc(i.id)}" data-original="${esc(q.price??'')}" value="${esc(q.price??'')}" placeholder="R$ 0,00"></label><label>Negociado<input aria-label="Preço negociado ${esc(s.name)}" inputmode="decimal" data-negotiated data-original="${esc(finalValue)}" value="${esc(finalValue)}" placeholder="R$ 0,00"></label></div><div class="quote-insight"><span class="discount-auto ${calc&&Number(calc.discount_percent)>0?'has-discount':''}">${calc?`${Number(calc.discount_percent).toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})}% de desconto`:'Sem cotação'}</span>${calc?`<span class="quote-saving">Economia ${brl(calc.saving)}</span>`:''}</div><div class="quote-total">${calc?`<span>Total do item</span><strong>${brl(calc.net)}</strong>`:'Sem cotação'}</div></div></td>`;}).join('')}<td class="winner-cell result-cell">${line.chosen?`<span class="result-label">Melhor proposta</span><strong>${esc(line.chosen.supplier)}</strong><span class="result-price">${brl(line.chosen.net)}</span>`:line.state==='tie'?`<span class="badge action-attention">Empate</span><small>Defina o fornecedor vencedor</small><select data-tie="${esc(i.id)}"><option value="">Escolher entre empatados</option>${line.winners.map(w=>`<option value="${w.supplier_id}">${esc(w.supplier)}</option>`).join('')}</select>`:'<span class="result-empty">Aguardando cotação</span>'}</td></tr>`;}).join('')}</tbody></table></div>
 </section>
...
function resultHtml(r,m) {
 const gross=Number(r.net)+Number(r.saving),defined=Math.max(0,m.items.length-r.unquoted-r.ties),completion=m.items.length?Math.round((defined/m.items.length)*100):0;
 const winners=r.suppliers.filter(s=>s.won);
 return `<section class="panel map-results-panel"><div class="results-heading"><div><span class="section-kicker">RESUMO DA NEGOCIAÇÃO</span><h2>Resultado da comparação</h2><p class="muted">Visão consolidada das melhores propostas já definidas no mapa.</p></div><div class="completion-ring" style="--progress:${completion}"><strong>${completion}%</strong><span>definido</span></div></div><div class="map-summary-grid"><div class="map-metric"><small>Valor inicial dos vencedores</small><strong>${brl(gross)}</strong><span>Base antes da negociação</span></div><div class="map-metric primary"><small>Valor negociado</small><strong>${brl(r.net)}</strong><span>${defined} de ${m.items.length} itens definidos</span></div><div class="map-metric success"><small>Economia obtida</small><strong>${brl(r.saving)}</strong><span>Desconto nos vencedores</span></div><div class="map-metric ${r.unquoted?'attention':''}"><small>Itens sem cotação</small><strong>${r.unquoted}</strong><span>${r.ties} ${r.ties===1?'empate pendente':'empates pendentes'}</span></div></div><div class="result-explainer">Economia considera o desconto negociado do fornecedor vencedor. Itens sem cotação e empates não resolvidos ficam fora do total. O mapa compara propostas; não gera OC nem confirma compra.</div>${winners.length?`<div class="supplier-results-head"><div><h3>Distribuição da compra</h3><p class="muted">Fornecedores que vencem pelo menos um item na comparação atual.</p></div><span>${winners.length} ${winners.length===1?'fornecedor':'fornecedores'}</span></div><div class="supplier-result-grid">${winners.map(s=>`<article class="supplier-result-card"><div class="supplier-result-title"><span class="supplier-result-mark">${esc((s.name||'?').trim().charAt(0).toUpperCase()||'?')}</span><div><h3>${esc(s.name)}</h3><p>${s.won} ${s.won===1?'item vencedor':'itens vencedores'}</p></div></div><div class="supplier-result-value"><small>Total definido</small><strong>${brl(s.net)}</strong></div><div class="supplier-result-saving"><span>Economia</span><strong>${brl(s.saving)}</strong></div><ul>${r.lines.filter(l=>l.chosen?.supplier_id===s.id).map(l=>{const i=m.items.find(i=>i.id===l.id);return `<li><span>${esc(i.description)}</span><small>${qty(i.quantity)} ${esc(i.unit)} · ${esc(i.company)} · SCI ${esc(i.sci)}</small></li>`;}).join('')}</ul></article>`).join('')}</div>`:'<div class="empty-result-state"><strong>Nenhum vencedor definido ainda.</strong><span>Preencha as cotações para visualizar a distribuição da compra.</span></div>'}</section>`;
}
