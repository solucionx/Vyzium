...
async function navigate(view) {
 ...
 if(view==='dashboard'||view==='items')await renderItems();if(view==='maps')await renderMaps();if(view==='history')await renderHistory();if(view==='settings')renderSettings();
}
...
function renderSettings() {
 $('#content').innerHTML=whatsappPanel()+`<section class="panel"><h2>Vyzium · Cotação &amp; Mapas · v3.0</h2><p>Dados, mapas e filtros deste módulo continuam salvos separadamente neste computador.</p><p class="muted">A conexão do WhatsApp pertence ao Vyzium e é reutilizada pelos dois módulos. As bases operacionais de Acompanhamento e Cotação &amp; Mapas continuam independentes.</p><p class="muted">A integração usa WhatsApp Web, sem API oficial. Alterações no serviço podem exigir reconexão.</p><label>Zoom<select id="zoom">${[75,85,89,100,110].map(v=>`<option value="${v}" ${(ui.zoom||85)===v?'selected':''}>${v}%</option>`).join('')}</select></label></section>`;
 startWhatsAppPanel();on($('#zoom'),'change',async()=>{ui.zoom=Number($('#zoom').value);await window.followup.setZoom(ui.zoom);await persist();});
}
