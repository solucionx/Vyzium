...
function on(el,event,fn) {el.addEventListener(event, e=>Promise.resolve().then(()=>fn(e)).catch(()=>{}));}
...
window.vyziumBeforeNavigateAway=async context=>{
 const destination=context?.type==='module'&&context?.target==='followup'?'Acompanhamento':context?.type==='home'?'Visão geral':'outra tela';
 return confirmUnsavedMap(destination);
};
...
function renderMap(detail) {
 ...
 <section class="panel"><div class="section-head"><div><h2>Mapa de cotação</h2><p class="muted">Informe o preço inicial e o preço unitário negociado. O desconto é calculado automaticamente. Sem negociação, deixe o segundo campo vazio. Frete não incluído.</p></div><button id="request-quote" class="button primary" ${!m.suppliers.length||m.archived||detail.changes.length?'disabled':''}>Solicitar cotação por WhatsApp</button></div>
...
async function renderSettings() {
 $('#content').innerHTML=whatsappPanel()+`<section class="panel"><h2>Segurança dos dados</h2><p>O banco deste módulo é verificado antes de qualquer migração e recebe snapshots consistentes antes de importações e atualizações.</p><div id="compras-data-safety" class="notice">Verificando banco de dados…</div><div class="toolbar"><button id="compras-backup-now" class="button primary">Criar backup agora</button><button id="compras-open-backups" class="button secondary">Abrir pasta de backups</button></div><p class="muted">Backups manuais não são removidos automaticamente. Os backups automáticos mais antigos são limitados para controlar o uso de disco.</p></section><section class="panel"><h2>Vyzium · Cotação &amp; Mapas · v3.0.3</h2><p>Dados, mapas e filtros deste módulo continuam salvos separadamente neste computador.</p><p class="muted">A conexão do WhatsApp pertence ao Vyzium e é reutilizada pelos dois módulos. As bases operacionais de Acompanhamento e Cotação &amp; Mapas continuam independentes.</p><p class="muted">A integração usa WhatsApp Web, sem API oficial. Alterações no serviço podem exigir reconexão.</p><label>Zoom<select id="zoom">${[75,85,89,100,110].map(v=>`<option value="${v}" ${(ui.zoom||85)===v?'selected':''}>${v}%</option>`).join('')}</select></label></section>`;
 startWhatsAppPanel();
 on($('#zoom'),'change',async()=>{ui.zoom=Number($('#zoom').value);await window.followup.setZoom(ui.zoom);await persist();});
 const loadSafety=async()=>{try{const state=await api('GET','/data-safety');const last=state.last_backup;const when=last?.created_at?new Date(last.created_at).toLocaleString('pt-BR'):'Nenhum backup registrado';const warning=last&&last.valid_now===false?'<br><small>⚠ O último backup não passou na revalidação atual. Crie um novo backup antes de operações críticas.</small>':'';$('#compras-data-safety').innerHTML=`<strong>${state.integrity?.ok?'✓ Banco íntegro':'⚠ Verificação requer atenção'}</strong><br>Schema ${esc(state.schema_version??'—')} · ${esc(state.backup_count??0)} backup(s)<br><small>Último backup: ${esc(when)}${last?.reason?` · ${esc(last.reason)}`:''}</small>${warning}`;}catch(e){$('#compras-data-safety').textContent='Não foi possível verificar a segurança do banco.';}};
 ...
 on($('#compras-open-backups'),'click',async()=>{try{await window.followup.openBackupFolder();}catch(e){toast('Não foi possível abrir a pasta de backups.');}});
 await loadSafety();
}
